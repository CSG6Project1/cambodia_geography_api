...
const linkPaginate = (increase, decrease, listQuery, first, last) => {
    let base_URL = `${process.env.HOST}/filterplaces/result`
    let sign = '?'
    ...
    asyncHandler(async (req, res) => {
        ...
        if(final_result.length!=0){
            ...
            if (req.query['province_code']) {
                listQuery.push({ name: 'province_code', value: req.query['province_code'] })
            }
            if (req.query['district_code']) {
                listQuery.push({ name: 'district_code', value: req.query['district_code'] })
            }
            if (req.query['commune_code']) {
                listQuery.push({ name: 'commune_code', value: req.query['commune_code'] })
            }
            if (req.query['village_code']) {
                listQuery.push({ name: 'village_code', value: req.query['village_code'] })
            }
