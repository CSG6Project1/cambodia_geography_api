...
const linkPaginate = (increase, decrease, listQuery, first, last) => {
    let base_URL = `${process.env.HOST}/api/provinces`
    let sign = '?'
...
const getProvinceDetail = asyncHandler(async (req, res) => {
  ...
  try {
    const province = await Province.findById(id).populate('comments')
    if (province) {
      res.send({ province })
    } else {
    ...
    res.send({
      message: 'Province not found Invalid Id',
    })
    return
  }
})
...
const deleteProvince = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (place) {
      if (Object.keys(province.images).length !== 0) {
        province.images.forEach((img) => {
          cloudinary.uploader.destroy(img.id)
        })
      }
      await province.comments.map(async (comment) => {
...
const createProvince = (req, res) => {
  uploadImage(
    ...
    asyncHandler(async (err) => {
      ...
      try {
        const province = await Province.create(createQuery)
        if (req.files) {
          req.files.forEach(async (file) => {
            const addImg = await Province.findById(province._id)
  ...
  uploadImage(
    ...
    asyncHandler(async (err) => {
      ...
      try {
        ...
        if (req.files) {
          req.files.forEach(async (file) => {
            const updateProvince = await Province.findById(id)
            cloudinary.uploader.upload(
              file.path,
              { folder: 'images' },
              async (error, result) => {
                if (error) {
                  ...
                  updateProvince.images.push(img)
                  updateProvince.save()
                }
              }
            )
          ...
          const province = await Province.findByIdAndUpdate(id, updateQuery, {
            new: true,
          })
          res.send({
...
export { getProvinces, getProvinceDetail, deleteProvince, updateProvince, createProvince }
